...
// Base classes used
//
// CBaseRenderer        Base renderer class supporting IMediaFilter
...
// Overriden to open the dump file
//
STDMETHODIMP CDump::Run(REFERENCE_TIME tStart)
{
    CAutoLock cObjectLock(m_pLock);
...
//
HRESULT CDump::WriteStringInfo(IMediaSample *pSample)
{
...
//    CUnknown(NAME("CDump"), pUnk),
	CBaseRenderer(CLSID_Dump, NAME("CDump"), pUnk, phr),
...
HRESULT CDump::CheckMediaType(const CMediaType *)
{
	return S_OK;
}


HRESULT CDump::DoRenderSample(IMediaSample *pSample)
{
	CheckPointer(pSample,E_POINTER);
	...
	return Write(pbData, pSample->GetActualDataLength());
}
